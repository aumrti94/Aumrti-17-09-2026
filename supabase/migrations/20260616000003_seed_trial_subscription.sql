-- ═══════════════════════════════════════════════════════════════════════════
-- Trial subscription auto-assignment
--
-- Root cause fixed: hospitals created before migration 20260601150001 (which
-- introduced hospital_subscriptions) never got a subscription row, so the
-- Platform Overview dashboard shows them as "no subscription" / plan "—" and
-- useSubscriptionConfig() treats them as fully-permissive (no real entitlement
-- enforcement). New registrations already get a row via register-hospital
-- edge function (index.ts:213-218) — this migration closes the gap for
-- everyone else and makes it automatic going forward.
--
-- Part A: Extend seed_hospital_defaults() with STEP 7 — assigns the default
--         "starter" plan with a trial period, mirroring register-hospital's
--         logic exactly. Idempotent (ON CONFLICT hospital_id DO NOTHING), so
--         hospitals that already have a subscription (e.g. from the edge
--         function) are left untouched.
-- Part B: Backfill all existing hospitals by re-running seed_hospital_defaults
--         for each one — same pattern as prior STEP additions.
--
-- Because seed_hospital_defaults() already fires AFTER INSERT ON hospitals
-- (trigger trg_seed_hospital_defaults, migration 20260613110000), STEP 7
-- also covers every future hospital automatically — no separate trigger
-- needed.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  VALUES
    (p_hospital_id,'CBC',                      'CBC',   'hematology',   'blood',NULL,    NULL, NULL,  NULL,  NULL,  60,  0, true),
    (p_hospital_id,'Haemoglobin',              'HB',    'hematology',   'blood','g/dL',  12.0, 17.5,   7.0,  20.0,  60,  0, true),
    (p_hospital_id,'WBC (Total Leukocyte Count)','TLC', 'hematology',   'blood','x10³/μL',4.0, 11.0,   2.0,  30.0,  60,  0, true),
    (p_hospital_id,'Platelet Count',           'PLT',   'hematology',   'blood','x10³/μL',150, 400,   50.0,1000.0,  60,  0, true),
    (p_hospital_id,'RBC Count',                'RBC',   'hematology',   'blood','x10⁶/μL',4.0, 5.5,   2.0,   7.0,  60,  0, true),
    (p_hospital_id,'PCV / Haematocrit',        'PCV',   'hematology',   'blood','%',     36.0, 50.0,  20.0,  60.0,  60,  0, true),
    (p_hospital_id,'MCV',                      'MCV',   'hematology',   'blood','fL',    80.0,100.0,  60.0, 120.0,  60,  0, true),
    (p_hospital_id,'MCH',                      'MCH',   'hematology',   'blood','pg',    27.0, 33.0,  15.0,  45.0,  60,  0, true),
    (p_hospital_id,'MCHC',                     'MCHC',  'hematology',   'blood','g/dL',  32.0, 36.0,  20.0,  40.0,  60,  0, true),
    (p_hospital_id,'ESR',                      'ESR',   'hematology',   'blood','mm/hr',  0.0, 15.0,  NULL, 100.0,  60,  0, true),
    (p_hospital_id,'PT / INR',                 'INR',   'hematology',   'blood','INR',    0.9,  1.1,  NULL,   5.0, 120,  0, true),
    (p_hospital_id,'LFT',                      'LFT',   'biochemistry', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 120,  0, true),
    (p_hospital_id,'SGOT (AST)',               'SGOT',  'biochemistry', 'blood','U/L',    0.0, 40.0,  NULL, 500.0, 120,  0, true),
    (p_hospital_id,'SGPT (ALT)',               'SGPT',  'biochemistry', 'blood','U/L',    0.0, 40.0,  NULL, 500.0, 120,  0, true),
    (p_hospital_id,'Alkaline Phosphatase',     'ALP',   'biochemistry', 'blood','U/L',   44.0,147.0,  NULL,1000.0, 120,  0, true),
    (p_hospital_id,'Bilirubin Total',          'TBIL',  'biochemistry', 'blood','mg/dL',  0.2,  1.2,  NULL,  15.0, 120,  0, true),
    (p_hospital_id,'Bilirubin Direct',         'DBIL',  'biochemistry', 'blood','mg/dL',  0.0,  0.3,  NULL,  10.0, 120,  0, true),
    (p_hospital_id,'Total Protein',            'TP',    'biochemistry', 'blood','g/dL',   6.0,  8.3,   3.0,  12.0, 120,  0, true),
    (p_hospital_id,'Albumin',                  'ALB',   'biochemistry', 'blood','g/dL',   3.5,  5.0,   2.0,   6.0, 120,  0, true),
    (p_hospital_id,'KFT',                      'KFT',   'biochemistry', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 120,  0, true),
    (p_hospital_id,'Serum Creatinine',         'CREAT', 'biochemistry', 'blood','mg/dL',  0.6,  1.2,  NULL,  10.0, 120,  0, true),
    (p_hospital_id,'Blood Urea',               'BU',    'biochemistry', 'blood','mg/dL',  7.0, 25.0,  NULL, 100.0, 120,  0, true),
    (p_hospital_id,'Uric Acid',                'UA',    'biochemistry', 'blood','mg/dL',  2.5,  7.0,  NULL,  13.0, 120,  0, true),
    (p_hospital_id,'Lipid Profile',            'LIPID', 'biochemistry', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 120,  0, true),
    (p_hospital_id,'Total Cholesterol',        'CHOL',  'biochemistry', 'blood','mg/dL',  0.0,200.0,  NULL, 400.0, 120,  0, true),
    (p_hospital_id,'HDL Cholesterol',          'HDL',   'biochemistry', 'blood','mg/dL', 40.0, 60.0,  20.0,  NULL, 120,  0, true),
    (p_hospital_id,'LDL Cholesterol',          'LDL',   'biochemistry', 'blood','mg/dL',  0.0,130.0,  NULL, 300.0, 120,  0, true),
    (p_hospital_id,'Triglycerides',            'TG',    'biochemistry', 'blood','mg/dL',  0.0,150.0,  NULL, 500.0, 120,  0, true),
    (p_hospital_id,'HbA1c',                    'HBA1C', 'biochemistry', 'blood','%',      4.0,  5.7,  NULL,  15.0, 120,  0, true),
    (p_hospital_id,'Blood Sugar Fasting',      'BSF',   'biochemistry', 'blood','mg/dL', 70.0,100.0,  40.0, 500.0,  60,  0, true),
    (p_hospital_id,'Blood Sugar PP',           'BSPP',  'biochemistry', 'blood','mg/dL', 70.0,140.0,  40.0, 600.0,  60,  0, true),
    (p_hospital_id,'Blood Sugar Random',       'BSR',   'biochemistry', 'blood','mg/dL', 70.0,140.0,  40.0, 600.0,  60,  0, true),
    (p_hospital_id,'TSH',                      'TSH',   'biochemistry', 'blood','mIU/L',  0.5,  5.0,   0.1, 100.0, 240,  0, true),
    (p_hospital_id,'T3 (Total)',               'T3',    'biochemistry', 'blood','ng/dL', 80.0,200.0,  NULL, 500.0, 240,  0, true),
    (p_hospital_id,'T4 (Total)',               'T4',    'biochemistry', 'blood','µg/dL',  5.0, 12.0,  NULL,  20.0, 240,  0, true),
    (p_hospital_id,'Serum Electrolytes',       'ELEC',  'biochemistry', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 120,  0, true),
    (p_hospital_id,'Serum Sodium (Na)',        'NA',    'biochemistry', 'blood','mEq/L', 136.0,145.0, 120.0, 160.0,  60,  0, true),
    (p_hospital_id,'Serum Potassium (K)',      'K',     'biochemistry', 'blood','mEq/L',  3.5,  5.0,   2.5,   6.5,  60,  0, true),
    (p_hospital_id,'Serum Chloride (Cl)',      'CL',    'biochemistry', 'blood','mEq/L', 98.0,107.0,  80.0, 120.0, 120,  0, true),
    (p_hospital_id,'CRP (C-Reactive Protein)', 'CRP',   'biochemistry', 'blood','mg/L',   0.0,  5.0,  NULL, 100.0, 120,  0, true),
    (p_hospital_id,'Dengue NS1 Antigen',       'DNS1',  'microbiology', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 240,  0, true),
    (p_hospital_id,'Widal Test',               'WIDAL', 'microbiology', 'blood',NULL,    NULL, NULL,  NULL,  NULL, 240,  0, true),
    (p_hospital_id,'Urine R/M',               'URM',   'urine',        'urine',NULL,    NULL, NULL,  NULL,  NULL,  60,  0, true),
    (p_hospital_id,'Urine Culture',           'UCUL',  'microbiology', 'urine',NULL,    NULL, NULL,  NULL,  NULL,2880,  0, true),
    (p_hospital_id,'ECG',                     'ECG',   'cardiology',   'other',NULL,    NULL, NULL,  NULL,  NULL,  15,  0, true)
  ON CONFLICT ON CONSTRAINT lab_test_master_hospital_id_test_name_key DO NOTHING;

  -- ── STEP 5: TPA / Insurer master catalog ──────────────────────────────
  INSERT INTO public.tpa_config
    (hospital_id, tpa_name, tpa_code, credit_days, required_documents, is_active)
  VALUES
    (p_hospital_id,'Star Health Insurance',   'STAR',      45,
       ARRAY['Admission letter','Investigation reports','Pre-auth form','Policy card copy'], true),
    (p_hospital_id,'New India Assurance',     'NIA',       60,
       ARRAY['Admission letter','Investigation reports','Claim form','Discharge summary'], true),
    (p_hospital_id,'United India Insurance',  'UII',       60,
       ARRAY['Admission letter','Investigation reports','Claim form'], true),
    (p_hospital_id,'ICICI Lombard',           'ICICI',     45,
       ARRAY['Admission letter','Investigation reports','Pre-auth form'], true),
    (p_hospital_id,'HDFC ERGO',               'HDFC',      45,
       ARRAY['Admission letter','Investigation reports','Pre-auth form'], true),
    (p_hospital_id,'Bajaj Allianz',           'BAJAJ',     45,
       ARRAY['Admission letter','Investigation reports','Pre-auth form','Policy card copy'], true),
    (p_hospital_id,'Medi Assist TPA',         'MEDASSIST', 30,
       ARRAY['Admission letter','Investigation reports','Pre-auth form'], true),
    (p_hospital_id,'MD India',                'MDINDIA',   30,
       ARRAY['Admission letter','Investigation reports','Pre-auth form'], true),
    (p_hospital_id,'Vidal Health TPA',        'VIDAL',     45,
       ARRAY['Admission letter','Investigation reports','Pre-auth form'], true),
    (p_hospital_id,'PMJAY / Ayushman Bharat', 'PMJAY',     21,
       ARRAY['Admission letter','PMJAY card','Pre-auth form','Aadhar copy'], true),
    (p_hospital_id,'CGHS',                    'CGHS',      30,
       ARRAY['Admission letter','CGHS card','Investigation reports','Referral letter'], true),
    (p_hospital_id,'ECHS',                    'ECHS',      30,
       ARRAY['Admission letter','ECHS card','Investigation reports','Referral letter'], true)
  ON CONFLICT ON CONSTRAINT tpa_config_hospital_id_tpa_code_key DO NOTHING;

  -- ── STEP 6: AI provider config ────────────────────────────────────────
  INSERT INTO public.ai_provider_config
    (hospital_id, feature_key, provider, model_name, is_active, temperature)
  VALUES
    (p_hospital_id, 'global_default',       'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'voice_scribe',         'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'radiology_impression', 'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'ai_digest',            'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'appeal_letter',        'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'discharge_summary',    'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'pre_auth_summary',     'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'pre_auth_ai_fill',     'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'denial_prediction',    'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'clinical_summary',     'claude', 'claude-sonnet-4-20250514', true, 0.3),
    (p_hospital_id, 'lab_interpretation',   'claude', 'claude-sonnet-4-20250514', true, 0.3)
  ON CONFLICT ON CONSTRAINT ai_provider_config_hospital_id_feature_key_key DO NOTHING;

  -- ── STEP 7: Trial subscription ──────────────────────────────────────
  -- Assigns the default "starter" plan with a trial period, mirroring
  -- register-hospital edge function's logic (index.ts:209-227). Idempotent:
  -- ON CONFLICT (hospital_id) DO NOTHING leaves hospitals that already have
  -- a subscription (e.g. from the edge function) untouched.
  ...
  IF v_plan_id IS NOT NULL THEN
    INSERT INTO public.hospital_subscriptions (hospital_id, plan_id, status, trial_ends_at)
    VALUES (p_hospital_id, v_plan_id, 'trial', now() + (COALESCE(v_trial_days, 30) || ' days')::interval)
    ON CONFLICT (hospital_id) DO NOTHING
    RETURNING hospital_id INTO v_inserted;

    IF v_inserted IS NOT NULL THEN
      -- Best-effort audit log. Wrapped defensively: the live subscription_events
      -- schema has already been found to differ from the local migration that
      -- defines it (real columns: subscription_id/from_state/to_state/actor/
      -- reason/razorpay_payload, not old_status/new_status/old_plan_id/
      -- new_plan_id/metadata) — never let an audit-log mismatch block the
      -- actual subscription row from being created.
      BEGIN
        INSERT INTO public.subscription_events (hospital_id, event_type, to_state, actor, reason)
        VALUES (p_hospital_id, 'trial_started', 'trial', 'system',
                'Backfilled missing subscription (seed_hospital_defaults STEP 7)');
...
-- ── Part B: Backfill existing hospitals ───────────────────────────────────
-- Re-runs seed_hospital_defaults for every hospital — fully idempotent
-- (all STEPs use ON CONFLICT DO NOTHING), so this only fills the gap for
-- hospitals missing a subscription; everyone else is a no-op.
DO $$
DECLARE
  r RECORD;
BEGIN
