-- Billing gap-fill plan, Phase 2 — refund payouts (both the pharmacy-return
-- pipeline and billing-desk cash refunds) never post a reversing journal entry:
-- no auto_posting_rules row exists for a "refund_payout" trigger event, so
-- RefundApprovalsInbox's approval step would silently fail to journalise the
-- cash leaving the hospital (logged only to accounting_posting_failures,
-- which nothing surfaces). Re-defining seed_hospital_defaults() to add the
-- missing rule (Dr Accounts Receivable / Cr Cash — reverses the original
-- bill_payment_* recognition), then re-running it for every existing
-- hospital to backfill — safe because every insert in the function is
-- ON CONFLICT DO NOTHING.
--
-- Note: there is no CHECK constraint on auto_posting_rules.trigger_event
-- (dropped in 20260613110000_seed_hospital_defaults.sql and never
-- re-added), so no ALTER TABLE is needed here — just the new seed row.
...
BEGIN
  ...
  IF v_ar IS NOT NULL AND v_cash IS NOT NULL AND v_opd_rev IS NOT NULL THEN
    ...
    VALUES
      ...
      -- Return / credit-note reversal (Dr Revenue / Cr AR — reverses revenue recognition)
      (p_hospital_id,'Pharmacy Return Credit Note',  'credit_note_pharmacy',
         COALESCE(v_pharm_ip, v_opd_rev), v_ar, 'Pharmacy Return - {description}', true),
      ...
      -- Refund payout (Dr Patient AR / Cr Cash — reverses bill_payment_* recognition)
      (p_hospital_id,'Patient Refund Payout',        'refund_payout',
         v_ar, v_cash, 'Refund - {description}', true),
...
-- Backfill: re-run for every existing hospital. Every insert above is
-- ON CONFLICT DO NOTHING, so this only adds the new refund_payout rule for
-- hospitals already seeded — nothing else changes.
DO $$
DECLARE
  h record;
BEGIN
  FOR h IN SELECT id FROM public.hospitals LOOP
    PERFORM public.seed_hospital_defaults(h.id);
  END LOOP;
END$$;
