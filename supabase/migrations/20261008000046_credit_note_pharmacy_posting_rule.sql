-- Pharmacy gap-fill plan, Phase 4 — the pharmacy return/refund flow posts a
-- reversing journal entry via autoPostJournalEntry({ triggerEvent: "credit_note_pharmacy" }),
-- but no auto_posting_rules row was ever seeded for that trigger event, so the
-- reversal silently failed (logged only to accounting_posting_failures, which
-- nothing surfaces). Re-defining seed_hospital_defaults() to add the missing
-- rule, then re-running it for every existing hospital to backfill — safe
-- because every insert in the function is ON CONFLICT DO NOTHING.
...
-- ON CONFLICT DO NOTHING, so this only adds the new credit_note_pharmacy
-- rule for hospitals already seeded — nothing else changes.
DO $$
DECLARE
  h record;
BEGIN
