-- ═══════════════════════════════════════════════════════════════════════════
-- TPA / Insurer master — per-hospital provisioning
--
-- Part A:  Add UNIQUE(hospital_id, tpa_code) to tpa_config, then extend
--          seed_hospital_defaults() with STEP 5 that seeds the 12 standard
--          TPA rows (exact list from migration 20260323043602).
...
-- ── Part A-1: Unique constraint (enables ON CONFLICT DO NOTHING) ──────────

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'tpa_config_hospital_id_tpa_code_key'
  ) THEN
    ALTER TABLE public.tpa_config
      ADD CONSTRAINT tpa_config_hospital_id_tpa_code_key
        UNIQUE (hospital_id, tpa_code);
  END IF;
END$$;

-- ── Part A-2: Extend seed_hospital_defaults (STEP 5 — TPA seeding added) ──
-- Full CREATE OR REPLACE; STEPs 1–4 are unchanged from 20260613110000.
...
BEGIN
  ...
  -- Mirrors the 12 rows from migration 20260323043602.
  -- ON CONFLICT DO NOTHING means hospitals that already have these rows
  -- (from the old all-or-nothing seed) are left untouched.
...
-- seed_hospital_defaults() is fully idempotent (all inserts use
-- ON CONFLICT DO NOTHING), so this block is safe to re-run at any time.
-- It seeds TPA rows for all hospitals that existed before this migration,
-- including any that were missed by the original 20260323043602 seed
-- (which used an all-or-nothing WHERE NOT EXISTS guard).
DO $$
DECLARE
  r RECORD;
BEGIN
