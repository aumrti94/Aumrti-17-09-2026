-- ═══════════════════════════════════════════════════════════════════════════
-- AI provider config — per-hospital provisioning
--
-- Part A:  Extend seed_hospital_defaults() with STEP 6 that seeds 11 feature
--          keys into ai_provider_config for every new hospital.
--          The UNIQUE(hospital_id, feature_key) constraint was created inline
--          in migration 20260327062736 — no ALTER TABLE needed.
-- Part B:  Backfill all existing hospitals.  The loop calls the updated
--          seed_hospital_defaults() which uses ON CONFLICT DO NOTHING
--          throughout — fully idempotent; safe to re-run.
--
-- NOTE:    The original one-time seed (20260327062736) only seeded 6 keys
--          (global_default, voice_scribe, radiology_impression, ai_digest,
--          appeal_letter, discharge_summary).  STEP 6 seeds all 11 so that
--          hospitals created after that migration get the full set, and the
--          backfill below adds the 5 missing rows for earlier hospitals.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── Part A: Extend seed_hospital_defaults (STEP 6 — AI config added) ──────
-- Full CREATE OR REPLACE; STEPs 1–5 are unchanged from 20260613150000.
...
BEGIN
  ...
  -- Seeds all 11 feature keys so every hospital gets working AI out of the
  -- box.  The original one-time seed (20260327062736) only seeded 6 keys;
  -- this backfills the 5 missing ones for older hospitals too.
  -- UNIQUE(hospital_id, feature_key) was created inline in 20260327062736
  -- and auto-named ai_provider_config_hospital_id_feature_key_key.
...
-- Adds the 5 new feature_key rows for hospitals seeded by 20260327062736,
-- and seeds all 11 rows for any hospital created after that migration but
-- before this one.  Fully idempotent — safe to re-run.
DO $$
DECLARE
  r RECORD;
BEGIN
