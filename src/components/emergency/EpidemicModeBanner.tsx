...
const EpidemicModeBanner: React.FC<Props> = ({ protocol }) => {
  ...
  const isMci = protocol.triage_mode === "mass_casualty";
  return (
    <div className={`flex-shrink-0 text-white px-4 py-2 flex items-center gap-3 z-50 ${isMci ? "bg-orange-600" : "bg-red-600"}`}>
      <AlertTriangle className="h-4 w-4 flex-shrink-0 animate-pulse" />
      <ShieldAlert className="h-4 w-4 flex-shrink-0" />
      <span className="text-sm font-bold uppercase tracking-wide">
        {isMci ? "🚨 MASS CASUALTY (MCI) MODE ACTIVE:" : "EPIDEMIC MODE ACTIVE:"}
      </span>
      <span className="text-sm font-semibold">{protocol.protocol_name}</span>
      <span className="text-xs opacity-80">|</span>
      {isMci
        ? <span className="text-xs font-semibold">START surge triage in effect</span>
